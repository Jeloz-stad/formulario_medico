...
def descargar_certificados():
    filename = "registros/certificado.xlsx"
    if os.path.exists(filename):
        with open(filename, "rb") as file:
            return file.read()
    return None
# valoracion medica
def descargar_valoracion():
    filename = "registros/valoracion_medica.xlsx"
    if os.path.exists(filename):
        with open(filename, "rb") as file:
            return file.read()
    return None
# densitometria
def descargar_densitometria():
    filename = "registros/densitometria.xlsx"
    if os.path.exists(filename):
        with open(filename, "rb") as file:
            return file.read()
    return None
# audiometria
def descargar_audiometria():
    filename = "registros/audiometria.xlsx"
    if os.path.exists(filename):
        with open(filename, "rb") as file:
            return file.read()
    return None
...
if st.session_state.current_form == "valoracion":
    ...
    with form_container:
        ...
        file_data = descargar_valoracion()
        if file_data:
            st.download_button(
                label="📥 Descargar Datos",
                data=file_data,
                file_name=f"valoracion_{datetime.now().strftime('%Y%m%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                help="Descarga todos los certificados almacenados"
            )
        else:
            st.warning("No hay datos almacenados")
...
# --- Formulario de DENSITOMETRÍA ---
    ...
    with form_container:
        ...
        # Botón de descarga fuera del formulario
        file_data = descargar_densitometria()
        if file_data:
            st.download_button(
                label="📥 Descargar Datos",
                data=file_data,
                file_name=f"densitometria_{datetime.now().strftime('%Y%m%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                help="Descarga todos los certificados almacenados"
            )
        else:
    ...
    with form_container:
        ...
        with st.form("form_audiometria"):
            ...
            if submitted:
                ...
                # Botón de descarga fuera del formulario
        file_data = descargar_audiometria()
        if file_data:
            st.download_button(
                label="📥 Descargar Datos",
                data=file_data,
                file_name=f"audiometria_{datetime.now().strftime('%Y%m%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                help="Descarga todos los certificados almacenados"
            )
        else:
    ...
    with form_container:
        with st.form("form_certificado"):
            ...
            if submitted:
                if ruc_certificado:
                    datos = {
                        "Fecha Emisión": fecha_emision.strftime("%Y-%m-%d"),
                        "Institución": institucion_certificado,
                        "RUC": ruc_certificado,
                        "CIU": ciu_certificado,
                        "Establecimiento": establecimiento_certificado,
                        "Historia Clínica": historia_certificado,
                        "Archivo": archivo_certificado,
                        "Cargo": cargo_certificado,
                        "Primer Apellido": primer_apellido_certificado,
                        "Segundo Apellido": segundo_apellido_certificado,
                        "Primer Nombre": primer_nombre_certificado,
                        "Segundo Nombre": segundo_nombre_certificado,
                        "Sexo": sexo_certificado,
                        "Evaluación": evaluacion,
                        "Estado Aptitud": estado,
                        "Condiciones Salud": condiciones,
                        "Observaciones": observaciones_certificado,
                        "Recomendaciones": recomendaciones_certificado,
                    ...
                    if archivo_guardado:
                        st.session_state.datos_guardados = archivo_guardado
                        st.success("¡Datos agregados exitosamente!")
                        # Forzar actualización del botón de descarga
                        st.rerun()
                else:
                    st.warning("Agregue el RUC por favor")
            # Botón de descarga fuera del formulario
        file_data = descargar_certificados()
        if file_data:
            st.download_button(
                label="📥 Descargar Datos",
                data=file_data,
                file_name=f"certificados_{datetime.now().strftime('%Y%m%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                help="Descarga todos los certificados almacenados"
            )
        else:
            st.warning("No hay certificados almacenados")
...
# Nota al pie
st.markdown("---")
st.caption("SISTEMA DE REGISTRO| FUNDACIÓN HOSPITALARIA SEMINARIO MENOR SAN LUÍS v2.0 | © 2025 |")
